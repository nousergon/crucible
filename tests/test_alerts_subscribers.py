...
alpha-engine-config-I10024: the v2 pages topic had ZERO subscribers from
...
def _topic() -> str:
    """Built lazily: `pages_topic()` reads an environment variable the
    autouse `conftest.py` fixture sets per test (`alpha-engine-config-
    I10156`), so a module-level constant would raise at collection time."""
    return f"arn:aws:sns:us-east-1:123456789012:{pages_topic()}"
...
def _sub(protocol: str, *, confirmed: bool = True) -> dict:
    return {
        "Protocol": protocol,
        "SubscriptionArn": (
            f"{_topic()}:00000000-0000-0000-0000-000000000001"
            if confirmed
...
class TestTheReading:
    def test_a_confirmed_email_and_a_lambda_leg_is_met(self) -> None:
        reading = pages_topic_subscribers(_topic(), sns=_Sns([_sub("email"), _sub("lambda")]))
        assert reading.met
    ...
    def test_a_pending_email_is_not_a_confirmed_one(self) -> None:
        ...
        reading = pages_topic_subscribers(
            _topic(), sns=_Sns([_sub("email", confirmed=False), _sub("lambda")])
        )
    ...
    def test_a_human_leg_without_the_lambda_leg_is_not_met(self) -> None:
        metric = pages_topic_subscribers(_topic(), sns=_Sns([_sub("email")])).metric(
            now=SATURDAY_NIGHT
        )
        assert metric["status"] == "FAIL"
    ...
    def test_no_subscribers_at_all_is_the_original_defect(self) -> None:
        metric = pages_topic_subscribers(_topic(), sns=_Sns([])).metric(now=SATURDAY_NIGHT)
        assert metric["status"] == "FAIL"
    ...
    def test_the_listing_is_paged_to_the_end(self) -> None:
        ...
        reading = pages_topic_subscribers(_topic(), sns=sns)
        assert reading.met
    ...
    def test_no_endpoint_address_reaches_the_metric(self) -> None:
        ...
        metric = pages_topic_subscribers(_topic(), sns=_Sns([sub, _sub("lambda")])).metric(
            now=SATURDAY_NIGHT
        )
    ...
    def test_a_denied_listing_raises_rather_than_reading_zero(self) -> None:
        with pytest.raises(PermissionError):
            pages_topic_subscribers(_topic(), sns=_Sns(fail=PermissionError("AccessDenied")))
...
class TestTheHeartbeatCarriesIt:
    def test_the_row_is_on_the_summary_and_validates_on_a_manifest(
        ...
        monkeypatch.setenv(PAGES_TOPIC_ARN_VAR, _topic())
