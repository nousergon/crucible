...
from crucible.report import ROWS, attribution_key, build_attribution
...
class TestCoverageRow:
    ...
    def test_a_session_with_no_run_counts_as_zero_and_is_named(self, tmp_path) -> None:
        """The mean over the days that DID run reads best when the most days
        are missing — which is the coverage defect wearing a report card."""
        store = _store(tmp_path)
    ...
    def test_a_failed_data_run_covers_nothing(self, tmp_path) -> None:
        store = _store(tmp_path)
        ...
        document, _ = build_attribution(store, trading_day=DAY, now=NOW, run_id="R" * 26)
        assert document["rows"][0]["value"] == pytest.approx(0.0)
